...
class PreferenceCollator(DataCollatorMixin):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        # NOTE: modified for the padding method to ensure the length of chosen and rejected are the same
        # 先分别padding chosen和rejected
        chosen_input_ids_padded = pad(
            chosen_input_ids, padding_value=self.pad_token_id)
        rejected_input_ids_padded = pad(
            rejected_input_ids, padding_value=self.pad_token_id)
        chosen_attention_mask_padded = pad(
            chosen_attention_mask, padding_value=0)
        rejected_attention_mask_padded = pad(
            rejected_attention_mask, padding_value=0)

        # 确保chosen和rejected被padding到相同的最大长度，避免后续处理中出现长度不一致的问题
        if chosen_input_ids_padded.shape[1] != rejected_input_ids_padded.shape[1]:
            max_len = max(
                chosen_input_ids_padded.shape[1], rejected_input_ids_padded.shape[1])
            chosen_input_ids_padded = pad_to_length(
                chosen_input_ids_padded, max_len, pad_value=self.pad_token_id)
            rejected_input_ids_padded = pad_to_length(
                rejected_input_ids_padded, max_len, pad_value=self.pad_token_id)
            chosen_attention_mask_padded = pad_to_length(
                chosen_attention_mask_padded, max_len, pad_value=0)
            rejected_attention_mask_padded = pad_to_length(
                rejected_attention_mask_padded, max_len, pad_value=0)

        output["chosen_input_ids"] = chosen_input_ids_padded
        output["chosen_attention_mask"] = chosen_attention_mask_padded
        output["rejected_input_ids"] = rejected_input_ids_padded
        output["rejected_attention_mask"] = rejected_attention_mask_padded
