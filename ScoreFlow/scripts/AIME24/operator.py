...
from utils.path_utils import ensure_dir, workspace_path
from utils.common import TIME_TO_EXEC_CODE, MAX_OPEN_FILES
...
class Programmer(Operator):
    ...
    # 类级别的执行器，但增加工作进程数以支持并发
    # 如果希望每个实例独立，可以改为实例级别（在 __init__ 中创建）
    _executor = concurrent.futures.ProcessPoolExecutor(
        max_workers=MAX_OPEN_FILES)
    ...
    async def exec_code(self, code: str, timeout: int = TIME_TO_EXEC_CODE) -> tuple[str, str]:
        ...
        try:
            ...
            # 使用类级别的共享执行器，max_workers>1 允许并发执行多个任务
        ...
        async def _retryable_call(analysis_content):
            ...
            for i in range(3):
                ...
                if execution_status == "Success":
                    ...
                    # 保存错误代码文件：将出错的代码保存到以报错信息命名的文件夹
                    error_code_base_dir = workspace_path("error_code")
                    ensure_dir(error_code_base_dir)
                    # 使用错误信息创建目录名（替换特殊字符，限制长度）
                    error_dir_name = str(execution_result).replace(
                        '/', '_').replace('\\', '_').replace(':', '_')[:100]
                    error_dir = os.path.join(
                        error_code_base_dir, error_dir_name)
                    ensure_dir(error_dir)

                    # 生成文件名（使用时间戳和尝试编号）
                    timestamp = int(time.time() * 1000)  # 毫秒时间戳
                    target_file = os.path.join(
                        error_dir, f"code_attempt_{i + 1}_{timestamp}.py")
                    # 处理重名：若已存在则加序号
                    counter = 1
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to save error code file: {e}")
