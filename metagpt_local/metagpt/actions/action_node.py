...
class ActionNode:
    ...
    async def _aask_v1(
        ...
        self._raw_output = await self.llm.aask(prompt, system_msgs, images=images, timeout=timeout)
        content = self._raw_output.outputs[0].text
    ...
    async def simple_fill(
        ...
        if schema != "raw":
            ...
            self._raw_output = await self.llm.aask(prompt)
            self.content = self._raw_output.outputs[0].text
...
"""
    ...
    async def code_fill(
        ...
        prompt = context
        self._raw_output = await self.llm.aask(prompt, timeout=timeout)
        content = self._raw_output.outputs[0].text
    ...
    async def single_fill(self, context: str, images: Optional[Union[str, list[str]]] = None) -> Dict[str, str]:
        field_name = self.get_field_name()
        prompt = context
        self._raw_output = await self.llm.aask(prompt, images=images)
        content = self._raw_output.outputs[0].text
    ...
    async def xml_fill(self, context: str, images: Optional[Union[str, list[str]]] = None) -> Dict[str, Any]:
        ...
        self._raw_output = await self.llm.aask(context, images=images)
        content = self._raw_output.outputs[0].text
