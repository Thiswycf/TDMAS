        ...
        if self.needs_chat_template(self.model_name):
            ...
            for prompt in prompts:
                if isinstance(prompt, str):
                    ...
                    assert len(prompt) % 2 == 1, f"Multi-round conversation must have odd number of messages (user-assistant-user-...-user), got {len(prompt)} messages"
                    # Assert: last message must be from user
                    assert prompt[-1]["role"] == "user", f"Last message in multi-round conversation must be from 'user', got '{prompt[-1]['role']}'"
                    ...
                    raise TypeError(f"Prompt must be str or List[Dict[str, str]], got {type(prompt)}")
        else:
