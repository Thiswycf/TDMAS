...
from pydantic_core import to_jsonable_python as pydantic_to_jsonable_python
from tenacity import RetryCallState, RetryError, _utils


def to_jsonable_python(obj: Any) -> Any:
    """Convert object to JSON serializable format, with support for numpy types."""
    ...
    # Try to use pydantic's implementation for other types
    try:
        return pydantic_to_jsonable_python(obj)
    except (TypeError, ValueError):
        # If pydantic can't handle it, try basic types
        if hasattr(obj, '__dict__'):
            return {k: to_jsonable_python(v) for k, v in obj.__dict__.items()}
        if isinstance(obj, (list, tuple)):
            return [to_jsonable_python(item) for item in obj]
        if isinstance(obj, dict):
            return {to_jsonable_python(k): to_jsonable_python(v) for k, v in obj.items()}
