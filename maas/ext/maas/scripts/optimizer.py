...
class Optimizer:
    ...
    async def _optimize_graph_maas(self):
        ...
        data = self.data_utils.load_results(graph_path) # dataset/train/results.json

        operator_descriptions = self.graph_utils.load_operators_description_maas(self.operators) 
        precomputed_operator_embeddings = torch.stack([get_sentence_embedding(op_desc) for op_desc in operator_descriptions]).to(self.device) # embedding of operator description 
    ...
    async def test(self):
        ...
        score, avg_cost, total_cost, total_tokens = await self.evaluation_utils.evaluate_graph_test_maas(self, directory, is_test=True, params=params)

        # 传递真实的cost值和估算的token数量
        new_data = self.data_utils.create_result_data(self.round, score, avg_cost=avg_cost, total_cost=total_cost, token=total_tokens)
