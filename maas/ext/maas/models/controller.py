...
class MultiLayerController(torch.nn.Module):
    ...
    def forward(self, query, operators_embedding, selection_operator_names, log_path=None):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            probs_1d = probs.squeeze(0) # prob of each operator
            ...
            if layer_idx == 0:
                if any(name.lower() == "earlystop" for name in selected_names):
                ...
                elif not any("generate" in name.lower() for name in selected_names): # force to 'generate' if no generate in selected_names
                    try:
                        generate_idx = selection_operator_names.index("Generate")
                    except ValueError:
                        generate_idx = 0
                    selected_indices = torch.tensor([generate_idx], device=self.device)
                    selected_names = ["Generate"]
                elif "generate" not in selected_names[0].lower() and any("generate" in name.lower() for name in selected_names): # If the first item in the currently selected list is not `generate`, but there are still `generate` operators in the list, then move the first `generate` operator to the front to ensure that `generate` is executed first, and the order of the others is preserved.
