...
class BaseBenchmark(ABC):
    ...
    def save_results_to_csv(self, results: List[Tuple[Any, ...]], columns: List[str]):
        ...
        # 估算token数量（基于prediction字段长度，假设1个token约等于4个字符）
        total_tokens = 0
        if "prediction" in df.columns:
            total_tokens = df["prediction"].apply(lambda x: len(str(x)) // 4 if x else 0).sum()
        ...
        return avg_score, avg_cost, total_cost, total_tokens
    ...
    # async def evaluate_all_problems(self, data: List[dict], graph: Callable, max_concurrent_tasks: int = 30, repetitions: int = 4, is_textgrad: bool = False, fast_mode: bool = False):
    async def evaluate_all_problems(self, data: List[dict], graph: Callable, max_concurrent_tasks: int = 30, repetitions: int = 4, is_textgrad: bool = False, fast_mode: bool = True):
        ...
        for rep in range(1, repetitions + 1):
            ...
            batch_end = len(data) // (2000 if fast_mode else 1)
            for batch_start in range(0, batch_end, self.batch_size):
                ...
                if len(logprobs) > 0:
                    # 确保logprobs中的所有元素都是Tensor类型
                    tensor_logprobs = []
                    for lp in logprobs:
                        if isinstance(lp, torch.Tensor):
                            tensor_logprobs.append(lp)
                        else:
                            # 如果不是Tensor，转换为Tensor，并保留梯度
                            logger.warning(f"Expected Tensor but got {type(lp).__name__}, converting...")
                            tensor_logprobs.append(torch.tensor(lp, requires_grad=True))
                    
                    logprobs = torch.stack(tensor_logprobs).to(self.device)
                    ...
                    if loss.requires_grad:
                        ...
                        logger.info(f"Repetition {rep}: Process {(batch_start + 1) / batch_end * 100:.2f}% Batch {batch_start // self.batch_size + 1} Loss: {loss.item()}")
                    else:
                        logger.info(f"Repetition {rep}: Process {(batch_start + 1) / batch_end * 100:.2f}% Batch {batch_start // self.batch_size + 1} Loss does not require grad and was skipped.")
                else:
                    logger.info(f"Repetition {rep}: Process {(batch_start + 1) / batch_end * 100:.2f}% Batch {batch_start // self.batch_size + 1} skipped due to invalid logprobs.")
    ...
    async def run_evaluation(self, graph: Callable, va_list: List[int], is_test: bool, sample: int, is_textgrad: bool = False, max_concurrent_tasks: int = 30):
        ...
        if is_test == True:
            ...
            average_score, avg_cost, total_cost, total_tokens = self.save_results_to_csv(results, columns)
            logger.info(f"Average score on {self.name} dataset: {average_score:.5f}")
            logger.info(f"Average cost: {avg_cost:.5f}, Total cost: {total_cost:.5f}")
            logger.info(f"Total tokens: {total_tokens}")
                
            return average_score, avg_cost, total_cost, total_tokens
